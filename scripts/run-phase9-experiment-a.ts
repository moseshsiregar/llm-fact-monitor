...
/**
 * Phase 9 — Experiment A (OPEN_ENDED_DISCOVERY).
 *
 ...
 * orchestration logic is introduced here - this script only supplies the
 * user-approved inputs and then reports on what was persisted.
 *
 * User-approved inputs (verbatim):
 *   - Subject: "Andy Burnham"
 *   - Prompt: "Tell me about Andy Burnham." (OPEN_ENDED_DISCOVERY)
 *   - 3 monitored facts (same fact text as the original REAL pilot)
 *   - 4 REAL providers (openai, perplexity, claude, gemini)
 *   - 3 repetitions
 *
 * Run via `npx tsx scripts/run-phase9-experiment-a.ts`.
 */
...
const PROMPT_TEXT = "Tell me about Andy Burnham.";
...
async function main() {
  ...
  console.log("Creating and running Experiment A (OPEN_ENDED_DISCOVERY) via the production path...");
